"""Local web UI and JSON API for Traction.
...
class GripHTTPServer(ThreadingHTTPServer):
    daemon_threads = True
    ...
    def __init__(self, address: tuple[str, int], project: Path, static_root: Path):
        ...
        super().__init__(address, GripRequestHandler)


class GripRequestHandler(BaseHTTPRequestHandler):
    server: GripHTTPServer

    def log_message(self, fmt: str, *args: Any) -> None:
        if os.environ.get("TRACTION_QUIET") != "1":
...
def create_server(project: Path, host: str = "127.0.0.1", port: int = 8765, static_root: Path | None = None) -> GripHTTPServer:
    root = static_root or (Path(__file__).resolve().parent / "web_dist")
    return GripHTTPServer((host, port), project, root)
