---
name: pull
description: "Use after executable verification for independent read-only review of one sealed patch against one locked contract. In Traction mode, fill review-*.json and JUDGMENT.json; ./traction pull enforces role separation, complete criterion coverage, finding accounting, and evidence-based conflict closure."
...
## Inputs in Traction mode

When `./traction next` reports `REVIEWING`, every reviewer receives the same:
...
Detailed rubrics remain in `references/`. The generated artifact contract is authoritative during Traction runs.
