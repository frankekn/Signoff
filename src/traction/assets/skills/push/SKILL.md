---
name: push
description: "Use before costly implementation to obtain independent, falsifiable PROCEED / STOP / PIVOT advice on the same decision surface. In Traction mode, fill the generated PUSH.json and let ./traction lock validate quorum, hashes, verdict-split conflicts, and conflict closure."
---
...
## Traction runtime mode

When `./traction next` reports `PUSH`, use the generated `PUSH.json` as the output contract. Read the exact same sealed packet for every advisor:
...
General-purpose prompt references remain in `references/`, but Traction's generated JSON and hashes take precedence during a run.
