...
from .errors import IntegrityError, TractionError, StateError, ValidationError
...
class Runtime:
    ...
    def start(self, goal: str) -> dict[str, Any]:
        ...
        baseline = snapshot_commit(self.project, f"Traction run baseline {run_id}")
        state = {
            "schema_version": 1,
            "run_id": run_id,
            "phase": "DRAFT",
    ...
    def activate_slice(self) -> dict[str, Any]:
        ...
        baseline = snapshot_commit(self.project, f"Traction slice {state['current']['iteration']} baseline")
    ...
    def status(self) -> dict[str, Any]:
        ...
        if not run_id:
            return {"phase": "IDLE", "active_run_id": None, "next": "Run ./traction start \"<exact user-visible outcome>\""}
    ...
    def next_action(self) -> str:
        ...
        run_id = root.get("active_run_id")
        if not run_id:
            return 'Run ./traction start "<the user’s exact outcome>".'
        state = read_run_state(self.project, run_id)
        phase = state["phase"]
        base = run_dir(self.project, run_id).relative_to(self.project)
        slice_draft_action = "Complete the active CONTRACT.json, then run ./traction slice."
        if state.get("current"):
            slice_draft_action = (
                f"Complete {base}/iterations/{state['current']['iteration']:04d}/CONTRACT.json, "
                "then run ./traction slice."
            )
        actions = {
        ...
        return actions.get(phase, f"Unknown phase {phase}; run ./traction integrity and inspect STATE.json.")
