...
START = "<!-- signoff:managed:start -->"
END = "<!-- signoff:managed:end -->"
MANAGED_BLOCK = f"""{START}
## Signoff

This repository uses the local Signoff control plane.

1. Read `.agents/skills/signoff/SKILL.md` before changing product code.
2. Run `./signoff status` and obey the single action returned by `./signoff next`.
...
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$ROOT/.signoff/runtime${PYTHONPATH:+:$PYTHONPATH}" exec "$PYTHON" -m signoff --project "$ROOT" "$@"
'''

CMD_LAUNCHER = r'''@echo off
set "ROOT=%~dp0"
set "PYTHONDONTWRITEBYTECODE=1"
set "PYTHONPATH=%ROOT%.signoff\runtime;%PYTHONPATH%"
python -m signoff --project "%ROOT%" %*
'''

POWERSHELL_LAUNCHER = r'''$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$env:PYTHONDONTWRITEBYTECODE = "1"
$env:PYTHONPATH = "$Root\.signoff\runtime;$env:PYTHONPATH"
python -m signoff --project $Root @args
'''
...
def find_source_root() -> Path | None:
    ...
    for parent in here.parents:
        if (parent / "skills" / "signoff" / "SKILL.md").is_file() and (parent / "src" / "signoff").is_dir():
            return parent
    return None
...
def install(project: Path, source_root: Path | None = None) -> dict[str, Any]:
    ...
    if source_root and (source_root / "skills").is_dir():
    ...
    elif (Path(__file__).resolve().parent / "assets" / "skills" / "signoff" / "SKILL.md").is_file():
        ...
        raise SignoffError("cannot locate the bundled Signoff, Council, and Roast skills")
    ...
    for host_dir in (project / ".agents" / "skills", project / ".claude" / "skills", project / ".gemini" / "skills"):
        for skill_name in ("signoff", "council", "roast"):
    ...
    launchers = {
        project / "signoff": (POSIX_LAUNCHER, 0o755),
        project / "signoff.cmd": (CMD_LAUNCHER, None),
        project / "signoff.ps1": (POWERSHELL_LAUNCHER, None),
    }
    ...
    manifest_path = project / ".signoff" / "install-manifest.json"
    manifest = {
        "schema_version": 1,
