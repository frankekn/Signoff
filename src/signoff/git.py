...
def run_git(project: Path, args: list[str], *, check: bool = True, input_text: str | None = None) -> subprocess.CompletedProcess[str]:
    ...
    if check and proc.returncode != 0:
        ...
        raise SignoffError(f"git {' '.join(args)} failed: {detail}")
    return proc
...
def ensure_repository(project: Path, *, require_commit: bool = True) -> None:
    ...
    if inside.returncode != 0 or inside.stdout.strip() != "true":
        raise SignoffError(f"Signoff requires a Git working tree: {project}")
    if require_commit:
        head_proc = run_git(project, ["rev-parse", "--verify", "HEAD"], check=False)
        if head_proc.returncode != 0:
            raise SignoffError("Signoff requires at least one Git commit so scope has a stable baseline")
...
def _untracked_patch(project: Path, path: str) -> str:
    ...
    if proc.returncode not in (0, 1):
        raise SignoffError(f"cannot create patch for untracked file {path}: {proc.stderr.strip()}")
    return proc.stdout
...
def snapshot_commit(project: Path, message: str) -> str:
    ...
    temp_dir = git_dir.resolve() / "signoff"
    ...
    try:
        for args in (["read-tree", base], ["add", "-A", "--", "."]):
            ...
            if proc.returncode != 0:
                raise SignoffError(f"cannot create Signoff snapshot: {proc.stderr.strip() or proc.stdout.strip()}")
        ...
        if tree_proc.returncode != 0:
            raise SignoffError(f"cannot write Signoff snapshot tree: {tree_proc.stderr.strip()}")
        identity_env = env.copy()
        identity_env.update(
            {
                "GIT_AUTHOR_NAME": "Signoff",
                "GIT_AUTHOR_EMAIL": "signoff@local.invalid",
                "GIT_COMMITTER_NAME": "Signoff",
                "GIT_COMMITTER_EMAIL": "signoff@local.invalid",
            }
        )
        ...
        if commit_proc.returncode != 0:
            raise SignoffError(f"cannot create Signoff snapshot commit: {commit_proc.stderr.strip()}")
