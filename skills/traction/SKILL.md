...
Advisors answer independently before seeing peers. Fill the generated `PUSH.json`. The chair must synthesize the selected or merged route. Material verdict splits and chair overrides of unanimous advisors require evidence-based resolution. If independent contexts are unavailable, return `INSUFFICIENT_QUORUM`; do not imitate extra voices.
